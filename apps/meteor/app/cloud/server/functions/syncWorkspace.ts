...
const fetchSyncPayload = async ({ reconnectCheck = false }: { reconnectCheck?: boolean } = {}): Promise<
	Serialized<Cloud.SyncPayload> | undefined
> => {
	...
	if (!connectToCloud && !reconnectCheck) {
		throw new CloudWorkspaceConnectionError('Workspace is not connected to Cloud');
	}
...
export async function syncWorkspace(reconnectCheck = false) {
	try {
		const payload = await fetchSyncPayload({ reconnectCheck });
